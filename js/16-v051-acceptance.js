(() => {
  ...
  function run(ticks=1000){
    ...
    const digestA=digest(a),digestB=digest(b),deterministic=digestA===digestB,core=a.selfTest(),forbidden=[];
    ...
    let staleEpochRejected=false,duplicateRequestRejected=false;
    if(req){
      const provider=req.simulation.providerHint==="local-fallback"?"local":req.simulation.providerHint;
      const normal=makeDecisionResponse(req,provider,{type:ACTION.WAIT,payload:{}},{goal:"boundary_test",reason:"dedupe",plan:"wait"},.5,1);
      const stale=cloneJson(req);stale.runEpoch=Math.max(0,(a.state.runEpoch||1)-1);stale.deadlineTick=a.state.tick+10;
      const staleChecked=a.validator.validate(normal,stale,a.state);staleEpochRejected=!staleChecked.ok&&staleChecked.errors.includes("RUN_EPOCH_MISMATCH");
      const key=`${req.runEpoch}:${req.requestId}`;a.decisionRouter.completedRequestIds.add(key);
      const before=a.queue.items.length;
      const fakeTask={kind:"response",agent,request:req,providerId:provider,rawResponse:normal,latencyMs:0};
      a.decisionRouter.complete(a.state,fakeTask,a.queue);
      duplicateRequestRejected=a.queue.items.length===before&&(a.state.metrics.duplicateDecisionsRejected||0)>0;
    }

    const result={version:"0.5.1",seed,ticks:count,tests:{worldMutationBoundary:lifecycleRejected,uiMutationIsolation:(()=>{const before=a.state.stock.food,snap=a.worldStateBoundary.runtimeSnapshot();try{snap.camp.x=-1}catch{}return a.state.stock.food===before&&snap!==a.state})(),observationIsolation:obsIsolation,sessionIsolation:sessions.ok,stagingAcceptedLifecycle:lifecycleAccepted,stagingRejectedLifecycle:lifecycleRejected,deterministicWorld:deterministic,staleEpochRejected,duplicateRequestRejected,existingV05EmergentRoles:emergent,integrityGate:core.ok},forbiddenMutationResults:forbidden,roleDistribution:a.roleDistribution(),specialists:a.specialistCount(),roleTransitions:a.state.metrics.roleTransitions||0,integrity:core};
