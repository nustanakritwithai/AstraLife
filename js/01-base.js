...
function actionFailureClass(actionType,message,extra={}){
  ...
  if(/actor unavailable|agent unavailable|human unavailable/.test(text))return ACTION_FAILURE_CLASS.ACTOR_UNAVAILABLE;
  if(/unvalidated/.test(text))return ACTION_FAILURE_CLASS.UNVALIDATED;
  return ACTION_FAILURE_CLASS.PRECONDITION;
}
function humanCanAttempt(agent,capability){return agent?.role==="human"&&HUMAN_CAPABILITIES[`can${capability[0].toUpperCase()}${capability.slice(1)}`]===true}
