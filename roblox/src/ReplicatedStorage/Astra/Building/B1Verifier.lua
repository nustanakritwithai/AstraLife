...
local WorldModules = script.Parent.Parent:WaitForChild("World")
...
function B1Verifier.Run()
    ...
    -- I0.2 boundary: shared World SurfaceResolver preserves logical X/Z, returns
    -- finite physical Y, and with AstraBaseplate present matches baseplate top
    -- (not logical cell.height / requested Y).
    ...
    local yOk = resolved.Y == resolved.Y and resolved.Y ~= math.huge and resolved.Y ~= -math.huge
    if baseplate and baseplate:IsA("BasePart") then
        local expectedTop = baseplate.Position.Y + baseplate.Size.Y * 0.5
        yOk = yOk and math.abs(resolved.Y - expectedTop) <= 1e-3
        yOk = yOk and resolved.Y ~= sample.Y
    end
    check(
        resolved.X == 5
            and resolved.Z == 7
