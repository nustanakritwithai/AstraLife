...
local function inferRole(agent)
    ...
    if string.find(name, "scout", 1, true) or string.find(agent.Name, "นักสำรวจ", 1, true) then return Config.Roles.Scout end
    if string.find(name, "gather", 1, true) or string.find(agent.Name, "เก็บ", 1, true) then return Config.Roles.Gatherer end
    if string.find(name, "builder", 1, true) or string.find(agent.Name, "ก่อสร้าง", 1, true) then return Config.Roles.Builder end
...
local function debug(agent, key, value) agent:SetAttribute(key, value) end
...
local function invalidateReport(state, report, reason, broadcast)
    ...
    if broadcast then
        Communication.BroadcastStatus(state.agent, state.folders.agents, state.tick, "resource_missing", {
            observationId = report.observationId, resourceId = report.resourceId, resourceType = report.resourceType,
            position = report.position, reason = reason, provenance = "direct",
        }, Config)
    end
        ...
        if message.type == "resource_report" and state.role == Config.Roles.Gatherer then
            ...
            if not state.processedObservations[payload.observationId] then
                ...
                local report = {
                    ...
                    position = payload.position, from = message.from, confidence = Config.ReportedResourceConfidence,
                    receivedTick = state.tick, expiresTick = expiresTick,
                }
                state.resourceReport = report
                state.resourceReports[payload.resourceType] = report
                remember(state, "resource_report_received", {
                    sourceType = "communication", sourceAgentId = message.from, originalObservationId = payload.observationId,
                    resourceId = payload.resourceId, resourceType = payload.resourceType, position = payload.position, receivedTick = state.tick,
                }, 0.85)
                setBelief(state, "resource:" .. payload.resourceId, {resourceType = payload.resourceType, position = payload.position},
...
local function updateBeliefsFromObservation(state, observations)
    for _, observation in ipairs(observations.resources) do
        setBelief(state, "resource:" .. observation.resourceId, {resourceType = observation.subtype, position = observation.position},
        ...
        remember(state, "resource_seen", {
            sourceType = "direct", resourceId = observation.resourceId, resourceType = observation.subtype,
            position = observation.position, distance = observation.distance, observationId = observation.id,
        }, 0.65)
        ...
        for _, report in pairs(state.resourceReports) do
            if report.resourceId == observation.resourceId then
                ...
                Communication.BroadcastStatus(state.agent, state.folders.agents, state.tick, "resource_verified", {
                    observationId = observation.id, resourceId = observation.resourceId, resourceType = observation.subtype,
                    position = observation.position, confidence = 1.0, provenance = "direct",
                }, Config)
                removeReport(state, report)
    ...
    setBelief(state, "threat_nearby", #observations.threats > 0, #observations.threats > 0 and 0.99 or 0.8,
        state.agent.Name, "direct", state.tick + 4)
end
        ...
        if state.tick - lastTick >= 3 then
            ...
            if sent > 0 then
                ...
                remember(state, "resource_report_sent", {
                    observationId = observation.id, resourceId = observation.resourceId, resourceType = observation.subtype,
                    position = observation.position, recipients = sent,
                }, 0.8)
            end
        end
    end
...
local function collectResource(state, observation)
    if state.role ~= Config.Roles.Gatherer then state.folders.state:SetAttribute("P1_RoleGuards", true) return false end
    ...
    remember(state, "resource_carried", {sourceType = "self_action", resourceId = resource.Name, resourceType = resourceType, amount = accepted, position = resource.Position}, 0.9)
    ...
    Communication.BroadcastStatus(state.agent, state.folders.agents, state.tick, "resource_missing", {
        observationId = observation.id, resourceId = resource.Name, resourceType = resourceType,
        position = resource.Position, reason = "collected", provenance = "self_action",
    }, Config)
    ...
    task.delay(Config.ResourceRespawnSeconds, function()
        if resource.Parent then resource.Transparency = 0 resource.CanQuery = true resource:SetAttribute("Active", true) end
    end)
    return true
end
...
local function deliverMaterials(state)
    ...
    for _, resourceType in ipairs(ResourceEconomy.Types()) do
        ...
        if carried > 0 and missing > 0 then
            ...
            if accepted > 0 then state.inventory:Remove(resourceType, accepted) table.insert(delivered, resourceType .. "+" .. tostring(accepted)) end
        end
    end
    updateInventoryDebug(state)
    if #delivered > 0 then
...
local function depositResources(state)
    ...
    local snapshot, depositedTotal, summary = state.inventory:Snapshot(), 0, {}
...
local function executeGather(state, observations)
    ...
    if target and (not state.preferredResourceType or target.subtype == state.preferredResourceType) then
        ...
        else think(state, "เห็น " .. target.subtype .. " → กำลังไปเก็บ") directMove(state, target.position, Config.WalkSpeed) end
        return
    end
    local report = state.preferredResourceType and state.resourceReports[state.preferredResourceType] or state.resourceReport
    if report then
        if state.tick > report.expiresTick then state.expiredMessages += 1 invalidateReport(state, report, "expired", false) think(state, "ข่าว Resource หมดอายุ") return end
...
local function storagePosition(state) return Storage.GetDepositPosition(state.storage) end
...
local function executeRest(state)
    ...
    if (restPosition - state.root.Position).Magnitude > Config.SurvivalUseDistance then
        think(state, inShelter and "เหนื่อย → กลับ Shelter" or "เหนื่อย → กลับฐานพัก") pathMove(state, restPosition) return
    end
...
local function executeSocialize(state, observations)
    ...
    if nearest then
        if nearest.distance > Config.SocialDistance then think(state, "ต้องการสังคม → เข้าใกล้ Agent") pathMove(state, nearest.position) return end
...
local function executeWaitFor(state, resourceType)
    ...
    if (storagePosition(state) - state.root.Position).Magnitude > Config.SurvivalUseDistance then pathMove(state, storagePosition(state))
    else state.humanoid:MoveTo(state.root.Position) end
end
...
local function executeBuild(state)
    ...
    if not active then
        ...
        if not blueprint then think(state, "สิ่งปลูกสร้างหลักครบแล้ว") return end
        active = Construction.TryStart(state.agent, state.folders, Config, state.tick)
    end
    if not active then return end
    if (active.position - state.root.Position).Magnitude > Config.ArrivalDistance + 2 then think(state, "ไปจุดก่อสร้าง " .. active.blueprint.displayName) pathMove(state, active.position) return end
    local ok, status, detail = Construction.Step(state.agent, state.folders, Config, state.tick)
    if ok and status == "progress" then think(state, string.format("สร้าง %s → %d%%", active.blueprint.displayName, detail))
    elseif ok and status == "completed" then remember(state, "structure_built", {sourceType = "self_action", blueprint = active.blueprint.id, position = active.position}, 1.0) think(state, "สร้าง " .. active.blueprint.displayName .. " สำเร็จ")
    elseif status == "awaiting_materials" then think(state, "รอวัสดุที่ Build Site: " .. tostring(detail))
    elseif status == "too_far" and typeof(detail) == "Vector3" then pathMove(state, detail) end
end

local function updateStuck(state)
    local moved = (state.root.Position - state.lastPosition).Magnitude
    if state.targetPosition and (state.targetPosition - state.root.Position).Magnitude > Config.ArrivalDistance then state.stuckTicks = moved <= Config.StuckDistanceEpsilon and (state.stuckTicks + 1) or 0
...
local function executeGoal(state, goal, observations)
    ...
    elseif goal == "Communicate" then if state.role == Config.Roles.Scout then scoutReport(state, observations) else executeExplore(state) end
...
function Brain.Start(agent)
    ...
    for _, obj in ipairs(agent:GetDescendants()) do if obj:IsA("BasePart") then obj.Anchored = false end end
    if root:CanSetNetworkOwnership() then root:SetNetworkOwner(nil) end

    local state = {
        agent = agent, humanoid = humanoid, root = root, head = head, role = role, folders = folders,
        storage = Storage.FindOrCreate(folders, Config), memory = Memory.new(Config), belief = Belief.new(Config),
        inventory = Inventory.new(Config.CarryCapacity), thoughtLabel = createThoughtBubble(head), homePosition = root.Position,
        targetPosition = nil, lastPosition = root.Position, stuckTicks = 0, tick = WorldState.GetTick(Config), localDecisionTick = 0,
    ...
    task.spawn(function()
        while runningAgents[agent] == state and agent.Parent and humanoid.Health > 0 do
            ...
            for _, report in ipairs(expiredReports) do state.expiredMessages += 1 invalidateReport(state, report, "expired", false) end
