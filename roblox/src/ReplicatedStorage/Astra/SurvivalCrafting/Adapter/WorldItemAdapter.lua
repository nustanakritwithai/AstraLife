...
function WorldItemAdapter.Convert(receipt, options)
	...
	options = options or {}
	local rawTool = options.tool or { class = receipt.toolClass, tier = receipt.toolTier, efficiency = 1 }
	local efficiency = rawTool.efficiency
	if type(efficiency) ~= "number" then efficiency = 1 end
	-- I3: cap tool efficiency at ≤1 so Convert yields cannot exceed committed
	-- withdrawal policy (GatheringContracts still applies its own floor).
	efficiency = math.min(1, efficiency)
	local tool = {
		class = rawTool.class or receipt.toolClass,
		tier = rawTool.tier or receipt.toolTier,
	...
	copy.yields = yields
	copy.toolEfficiency = efficiency
	return yields, copy
end
