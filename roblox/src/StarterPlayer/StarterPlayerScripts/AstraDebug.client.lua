...
local function agentLine(agent)
    ...
    return string.format(
        "%s [%s] G=%s Carry=%s/%s H:%s T:%s E:%s S:%s",
        agent.Name,
...
while task.wait(0.5) do
    ...
    local lines = {
        ...
        string.format("Tick:%s  P1:%s  P2:%s  P3:%s  P4:%s", attr(state, "WorldTick", 0), attr(state, "P1Status", "?"), attr(state, "P2Status", "?"), attr(state, "P3Status", "?"), attr(state, "P4Status", "?")),
        "",
        ...
        string.format("P3 site:%s request:%s delivery:%s ready:%s wait:%s build:%s complete:%s", tostring(attr(state, "P3_SiteCreated", false)), tostring(attr(state, "P3_MaterialsRequested", false)), tostring(attr(state, "P3_PhysicalDelivery", false)), tostring(attr(state, "P3_AllMaterialsDelivered", false)), tostring(attr(state, "P3_BuilderWaited", false)), tostring(attr(state, "P3_BuildProgress", false)), tostring(attr(state, "P3_BuildCompleted", false))),
        string.format("P4 needs:%s goal:%s eat:%s drink:%s rest:%s social:%s critical:%s", tostring(attr(state, "P4_NeedsDecayed", false)), tostring(attr(state, "P4_SurvivalGoalObserved", false)), tostring(attr(state, "P4_EatObserved", false)), tostring(attr(state, "P4_DrinkObserved", false)), tostring(attr(state, "P4_RestObserved", false)), tostring(attr(state, "P4_SocialObserved", false)), tostring(attr(state, "P4_CriticalDamageObserved", false))),
    }
