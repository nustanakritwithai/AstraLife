...
local Climate = require(Astra.WorldSimClimate)
local Terrain = require(Astra.WorldSimTerrain)
local Disease = require(Astra.WorldSimDisease)
local RouteSecurity = require(Astra.WorldSimRouteSecurity)
...
local climateFolder = ensureFolder(simRoot, "Climate")
local terrainFolder = ensureFolder(simRoot, "Terrain")
local diseaseFolder = ensureFolder(simRoot, "Disease")
local routeSecurityFolder = ensureFolder(simRoot, "RouteSecurity")
...
simRoot:SetAttribute("Version", "WorldSimCorePack-0.6")
simRoot:SetAttribute("SourceReference", "Kingdom-sandbox")
...
local function processTick(tick)
    ...
    local climate = Climate.Update(folders.state, hydrologyFolder, climateFolder, tick, seed)
    local terrain = Terrain.Update(folders.agents, terrainFolder, 32, seed)
    local economy = Economy.Update(folders, marketFolder, climateFolder)
    ...
    local disease = Disease.Update(folders.agents, climateFolder, hydrologyFolder, settlementFolder, diseaseFolder, tick, seed)
    local routeSecurity = RouteSecurity.Update(routesFolder, threatsFolder, terrainFolder, settlementFolder, routeSecurityFolder)
    ...
    Metrics.Gauge(metricsFolder, "DroughtPressure", math.floor((climate.droughtPressure or hydrology.droughtPressure or 0) * 1000 + 0.5) / 10)
    Metrics.Gauge(metricsFolder, "SeasonIndex", climate.seasonIndex or 1)
    Metrics.Gauge(metricsFolder, "DiseasePressure", math.floor((disease.pressure or 0) * 1000 + 0.5) / 10)
    Metrics.Gauge(metricsFolder, "SickAgents", disease.sick or 0)
    Metrics.Gauge(metricsFolder, "RouteSecurityPressure", math.floor((routeSecurity.securityPressure or 0) * 1000 + 0.5) / 10)
    Metrics.Gauge(metricsFolder, "TerrainMappedAgents", terrainFolder:GetAttribute("MappedAgentCount") or 0)
    ...
    folders.state:SetAttribute("WorldSimSeason", climate.season)
    folders.state:SetAttribute("WorldSimDroughtPressure", math.floor((climate.droughtPressure or 0) * 1000 + 0.5) / 10)
    folders.state:SetAttribute("WorldSimDiseasePressure", math.floor((disease.pressure or 0) * 1000 + 0.5) / 10)
    folders.state:SetAttribute("WorldSimRouteSecurity", routeSecurityFolder:GetAttribute("SecurityState") or "Unknown")
...
print("[AstraLife] Kingdom-sandbox WorldSim systems attached passively to authoritative WorldTick")
