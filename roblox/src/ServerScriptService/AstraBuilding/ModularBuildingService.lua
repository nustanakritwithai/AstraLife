...
function ModularBuildingService.PreviewRoot(pieceType, requestedPosition, yawDegrees)
    ...
    -- Snap X/Z/yaw FIRST, then sample physical surface at the snapped footprint.
    -- Sampling before snap on uneven ground would project the wrong cell.
    -- Shared World SurfaceResolver defaults also exclude agents/resources/structures;
    -- pass renderFolder as an extra exclude so preview does not hit ghost pieces.
    local provisional = current.validator:SnapRoot(
        Vector3.new(requestedPosition.X, 0, requestedPosition.Z),
        yawDegrees
    )
    local surfaceY = SurfaceResolver.TopY(provisional.Position, {
        exclude = { current.renderFolder },
    })
    local y = surfaceY + definition.size.Y * 0.5
    -- Re-SnapRoot after Y is known so yaw/grid stay consistent with validator.
    local cframe = current.validator:SnapRoot(
        Vector3.new(provisional.Position.X, y, provisional.Position.Z),
        yawDegrees
    )
