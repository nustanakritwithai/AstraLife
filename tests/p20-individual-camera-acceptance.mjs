...
try{
  ...
  const result=await page.evaluate(()=>{
    ...
    const stableSnapshot=()=>{const snapshot=runtime.snapshot();delete snapshot.metadata.exportedAt;return JSON.stringify(snapshot)};
    const initialSnapshot=stableSnapshot();
    ...
    const afterFocusSnapshot=stableSnapshot();
    camera.fitToMap();
    ...
    const beforeThoughtState=stableSnapshot();
    ...
    const restoredThoughtState=stableSnapshot();
    const eps=.001;
