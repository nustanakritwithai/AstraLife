...
const result=await page.evaluate(()=>{
  ...
  runtime.resolver.resolveMove(runtime.state,A,act1);
  const wm1=AstraLifeP3.ensure(A.id);const p1=wm1.predictions.at(-1);const err1=p1.error.energyAbs;const after1=wm1.travel.loadPenalty;
  ...
  runtime.resolver.resolveMove(runtime.state,A,act2);
  const wm2=AstraLifeP3.ensure(A.id);const p2=wm2.predictions.at(-1);const err2=p2.error.energyAbs;

  const predictionRecorded=p1.status==='RESOLVED'&&p1.predicted.energyCost>0&&p1.actual.energyCost>0;
  const wm01=predictionRecorded&&after1<start&&err2<err1&&p2.adjustment.after<p2.adjustment.before;
  ...
  const saved=AgentStateBoundaryV051.persistent(A);const savedPenalty=saved.worldModel.travel.loadPenalty;
  ...
  const tests={WM01_predictionErrorAndAdjustment:wm01,predictionOutcomeRecorded:predictionRecorded,learningEpisodeRecorded:!!lesson,constructionEstimateAvailable:construction?.predictedProgress>0,stormRiskEstimateAvailable:Number.isFinite(storm?.risk),worldModelPersists:persistence,inspectorShowsPredictionOutcome:inspectorShowsPrediction,P0IntegrityStillPasses:runtime.selfTest().ok};
