...
try{
  ...
  await page.waitForFunction(()=>window.AstraColony&&window.AstraLifeFramePacing&&window.AstraLifeTurnAhead);
  const result=await page.evaluate(()=>{
    const scheduler=window.AstraLifeFramePacing;
    const colony=window.AstraColony;
    runtime.state.running=false;
    colony.reset('P18-EIGHT-SECOND-TICK-20260909');
    runtime.state.running=false;
    scheduler.reset();
    ...
    for(let i=0;i<80;i++)scheduler.advanceForTest(100);
    ...
    let halfway;for(let i=0;i<40;i++)halfway=scheduler.advanceForTest(100);
    ...
    const tests={
      cadenceIsOneTickPerEightSeconds:config.intervalMs===8000&&Math.abs(config.simulationTicksPerSecond-.125)<1e-9,
      authoritativeTickArrivedAfterEightSeconds:afterTick===startTick+1,
      ...
      walkAnimationRetimed:scheduler.animation?.walkFrameMs>=500&&scheduler.animation?.walkFrameMs<=800
    };
    return {ok:Object.values(tests).every(Boolean),tests,config,animation:scheduler.animation,halfway,visualHalf,startTick,afterTick,afterHalfTick,afterStep,afterRunTicks};
  });
